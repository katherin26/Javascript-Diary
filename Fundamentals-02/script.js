...
const calcTempAmplitudeBug = function (t1, t2) {
  const temps = t1.concat(t2);
  console.log(temps); // array concatenado
  ...
  for (let i = 0; i < temps.length; i++) {
    ...
    debugger; // Es un breakpoint para determinar donde ahi que aplicar el debugger.
    if (curTemp > max) max = curTemp;
    if (curTemp < min) min = curTemp;
  }
  console.log(max, min);
...
//console.log(amplitudeBug); // ===> Sources (Debugger paused)
